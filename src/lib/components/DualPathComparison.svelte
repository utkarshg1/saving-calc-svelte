<script lang="ts">
	...
	let { compare, inputs }: Props = $props();
...
<div class="space-y-4">
	<div class="grid grid-cols-1 gap-4" style="grid-template-columns: repeat({instruments.length}, minmax(0, 1fr));">
		{#each instruments as inst (instKey(inst))}
			<div
				class="relative overflow-hidden rounded-2xl border border-slate-200/80 bg-white p-5 dark:border-slate-700 dark:bg-slate-800/80 {isBest(inst) ? 'ring-2 ring-teal-400' : ''}"
			>
				{#if isBest(inst)}
					<span class="absolute right-3 top-3 rounded-full bg-teal-100 px-2 py-0.5 text-[10px] font-bold uppercase text-teal-700 dark:bg-teal-900/50 dark:text-teal-300">Best</span>
				{/if}
				...
				<div class="mt-4 space-y-3">
					...
					<div class="grid grid-cols-2 gap-3 text-sm">
						...
						<div><p class="text-xs text-slate-400">Principal</p><p class="font-mono-num font-semibold">{formatINR(inst.principalSaved)}</p></div>
						<div><p class="text-xs text-slate-400">Net Gains</p><p class="font-mono-num font-semibold text-emerald-600">{formatINR(inst.gainsEarned - inst.taxAmount)}</p></div>
	...
	<div class="space-y-2">
		<div
			...
			aria-live="polite"
		>
			<p class="text-xs text-slate-500 dark:text-slate-400">{sipVsRdSummary.label}</p>
			<p class="font-mono-num mt-1 text-lg font-semibold {sipExcessToneClasses(sipVsRdSummary.tone)}">
		...
		{#if stepupVsRdSummary}
			<div
				...
				aria-live="polite"
			>
				<p class="text-xs text-slate-500 dark:text-slate-400">{stepupVsRdSummary.label}</p>
				<p class="font-mono-num mt-1 text-lg font-semibold {sipExcessToneClasses(stepupVsRdSummary.tone)}">
		...
		{#if stepupVsSipSummary}
			<div
				...
				aria-live="polite"
			>
				<p class="text-xs text-slate-500 dark:text-slate-400">{stepupVsSipSummary.label}</p>
				<p class="font-mono-num mt-1 text-lg font-semibold {sipExcessToneClasses(stepupVsSipSummary.tone)}">
