...
export type SipExcessTone = 'sip' | 'rd' | 'neutral' | 'stepup';
...
export function gainsExcessSummary(
	gainsA: number,
	gainsB: number,
	labelA: string,
	labelB: string
): { label: string; amount: string; text: string; tone: SipExcessTone } {
	const excess = gainsA - gainsB;
	const label = `${labelA} gains vs ${labelB} (after tax)`;
	if (excess > 0) {
		const amount = `+${formatINR(excess)}`;
		return { label, amount, text: `${label}: ${amount}`, tone: 'sip' };
	}
